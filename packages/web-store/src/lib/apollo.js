 ...
 */
export const initOnContext = (ctx, serverAccessToken) => {
 ...
 */
const initApolloClient = (initialState, ctx, serverAccessToken = null) => {
...
export const withApollo = ({ ssr = false } = {}) => PageComponent => {
  const WithApollo = ({
    ...
    //add serverAccessToken to client side
  ...
  if (ssr || PageComponent.getInitialProps) {
    WithApollo.getInitialProps = async ctx => {
      const inAppContext = Boolean(ctx.ctx);
      //fetch accessToken
      const { req } = inAppContext ? ctx.ctx : ctx;
      let serverAccessToken = '';

      if (typeof window === 'undefined' && req.headers && req.headers.cookie) {
